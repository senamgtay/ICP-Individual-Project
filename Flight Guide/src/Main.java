...
public class Main {
    ...
    /**
     * hashmap contains parents of airports
     */
    static HashMap<String, String> parents = new HashMap<>();

    /**
     * hashmap of direct flights and their possible airlines
     */
    static HashMap<String, ArrayList<String>> airlines = new HashMap<>();
    ...
    /**
     * reads from the routes file and creates airlines hashmap of
     * direct flights and their possible airlines
     */
    public static void readAirlines() {
        ...
        while (sc.hasNext()) {
            ...
            if (airlines.containsKey(keyFlightPair)) {
                airlines.get(keyFlightPair).add(newAirlineCode);
            } else {
                ArrayList<String> possAirlines = new ArrayList<>();
                possAirlines.add(newAirlineCode);
                airlines.put(keyFlightPair, possAirlines);
            }
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < theSolutionPath.size() - 1; i++) {
            ...
            if (airlines.containsKey(theSolutionPath.get(i) + "," + theSolutionPath.get(i + 1))) {
                String thisAirline = airlines.get(theSolutionPath.get(i) + "," + theSolutionPath.get(i + 1)).get(0);
        ...
        System.out.println(airlinePath);
            ...
            while (endIndex < theSolutionPath.size()) {
                ...
                myWriter.println(endIndex + "." + airlinePath.get(startIndex)  + " from " + start + " to " + end + " 0 Stops.");
                count += 1;
                startIndex += 1;
                endIndex += 1;
            }
